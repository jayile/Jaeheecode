...
<script>
	 ...
	 function freeboard() {
		var lineValue = document.getElementById("board").getAttribute('data-value');
		$.ajax({
			url: 'serch.do',
			type: 'POST',
			data: queryString,
			...
			success: function() {
				alert("성공!");
				window.location.replace("slist.jsp");
			}
		});
	 }
...
<script>
	...
	function serch_ajax() {
		var queryString = $('#serch_frm').serialize();
		var sort = $('#serchKey').val();
		var serch = $('#serch').val();
		var re = 'slist.do?key='+sort+'&value='+serch;
		    window.location.replace(re);
		/*
		$.ajax({
			url: 'serch.do',
			type: 'POST',
			data: { key : sort, value : serch },
			dataType: 'text',
			success: function() {
				alert("??");
				window.location.replace("slist.jsp");
  ...
  <tbody>
    <c:forEach items="${list}" var="dto">
		<tr>
			...
			<td>
				...
				<a href="notice_view.do?bId=${dto.bId}">${dto.bTitle}
	...
	<nav aria-label="Page navigation example">
			<ul class="pagination justify-content-center">
	            ...
	            <c:otherwise>
	            	<li class="page-item">
	                	<a class="page-link" href="notice.do?page=1" aria-label="Previous">
	                  		<span aria-hidden="true">&laquo;</span>
	                	</a>
	            	</li>
				</c:otherwise>
				...
				<c:otherwise>
					<li class="page-item">
	            		<a class="page-link" href="notice.do?page=${page.curPage - 1}">Previous</a>
	            	</li>
				</c:otherwise>
				</c:choose>
	            ...
	            <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
					...
					<c:when test="${page.curPage == fEach}">
						...
						<c:otherwise>
	            <li class="page-item">
	            	<a class="page-link" href="notice.do?&page=${fEach}">${fEach}</a>
				...
				<c:otherwise>
					<li class="page-item">
	                	<a class="page-link" href="notice.do?page=${page.curPage + 1}">Next</a>
	            	</li>
				</c:otherwise>
				</c:choose>
				...
				<c:otherwise>
					<li class="page-item">
	                	<a class="page-link" href="notice.do?page=${page.totalPage}" aria-label="Next">
	                  	<span aria-hidden="true">&raquo;</span>
	                	</a>
	            	</li>
				</c:otherwise>
